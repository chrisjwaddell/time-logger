function nodeAttributes(node) {
    return {
        type: node.nodeName,
        name: node.nodeName + " " + ((node.id) ? "#" + node.id + node.className : node.className),
        children: nodeChildren(node)
    }
}

const elParent = document.querySelector("html")
let test = nodeAttributes(elParent)
...
// load the external data from a file
// d3.json("20221104.json", function (error, treeData) {
//     root = treeData[0];
//     root.x0 = height / 2;
//     root.y0 = 0;
//     update(root);
